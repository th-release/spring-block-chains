...
public class Chain {
    ...
    public Failable<Block, String> addBlock(List<String> data) {
        ...
        Block adjustmentBlock = getAdjustmentBlock();
    ...
    public Block getAdjustmentBlock() {
        ...
        return getLength() < Config.DIFFICULTY_ADJUSTMENT_INTERVAL
                ? Block.getGenesis()
                : this.blockChain.get((int) (currentLength - Config.DIFFICULTY_ADJUSTMENT_INTERVAL));
    }

    public Failable<Long, String> addToChain(Block _recvBlock) {
        Failable<Block, String> isValid = Block.isValidNewBlock(_recvBlock, getLatestBlock());
        if (isValid.isError())
            return Failable.error(isValid.getError());
